...
来源: TikTokDownloader-master/src/encrypt/aBogus.py
该实现经 TikTokDownloader 项目验证可用，无外部 JS 依赖。
"""
...
class ABogus:
    ...
    def __init__(self, user_agent: str = None, platform: str = None):
        ...
        self.ua_code = self.generate_ua_code(self.user_agent)
    ...
    def generate_args_code(self):
        a = []
        for j in range(24, -1, -8):
            a.append(self.__arguments[0] >> j)
        a.append(self.__arguments[1] / 256)
        a.append(self.__arguments[1] % 256)
        a.append(self.__arguments[1] >> 24)
        a.append(self.__arguments[1] >> 16)
        for j in range(24, -1, -8):
            a.append(self.__arguments[2] >> j)
        return [int(i) & 255 for i in a]

    def generate_method_code(self, method: str = "GET"):
        return self.sm3_to_array(self.sm3_to_array(method + self.__end_string))

    def generate_params_code(self, params: str):
    ...
    @classmethod
    def generate_browser_info(cls, platform: str = "Win32"):
    ...
    def get_value(self, url_params, method="GET", start_time=0, end_time=0) -> str:
        """生成 a_bogus 签名值

        对齐 TikTokDownloader 的 deal_url_params():
          params = urlencode(params, quote_via=quote)
          params += f"&a_bogus={self.ab.get_value(params, method)}"
        """
        if isinstance(url_params, dict):
            url_params = urlencode(url_params, quote_via=quote)
...
def generate_a_bogus(params, data="", user_agent=""):
    """便捷函数，兼容旧接口"""
    ab = ABogus(user_agent=user_agent) if user_agent else ABogus()
    return ab.get_value(params, "POST" if data else "GET")
